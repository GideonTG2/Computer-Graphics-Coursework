...
int main(void)
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(1024, 768, "Max Eastope Grahpics Coursework", NULL, NULL);
    ...
    // Add walls to objects vector
    Object wallobject;
    wallobject.name = "wall";
    for (unsigned int i = 0; i < 5; i++)
    {
        wallobject.position = wallPositions[i];
        wallobject.scale = glm::vec3(5.0f, 1.0f, 5.0f);
        wallobject.rotation = wallRotation[i];
        wallobject.angle = Maths::radians(wallAngles[i]);
